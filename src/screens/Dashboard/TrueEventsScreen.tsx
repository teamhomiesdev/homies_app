...
const TrueEventsScreen = ({ navigation }: any) => {
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      fetchTrueEventsPosts(1, true);
    }, [])
  );

  const fetchTrueEventsPosts = async (pageToFetch: number, isInitialRefresh: boolean = false) => {
    ...
    try {
      ...
      if (json && json.success && json.data?.posts) {
        ...
        Alert.alert('Error', json.message || 'Failed to retrieve true events posts.');
      }
    } catch (error) {
      console.error('Error fetching true events posts:', error);
  ...
  const loadMorePosts = () => {
    ...
    if (currentPage < totalPages && !loadingMore) {
      ...
      fetchTrueEventsPosts(nextPage, false);
    }
  };
  ...
  const renderEmptyComponent = () => {
    return (
      <View style={styles.emptyContainer}>
        <Text style={styles.emptyText}>No true events found at this time.</Text>
      </View>
    );
  };
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <View style={styles.header}>
        <Text style={styles.headerTitle}>True Events</Text>
        <TouchableOpacity
...
export default TrueEventsScreen;
